...
function print_image(image, offset = 0) {
	if (!image) return;
...
function print_result(string = "", numbers = []) {
	...
	while (remain) {
		if (remain.startsWith("&#8862")) {
			...
			remain = remain.replace("}", "");
			offset += 4;
		} else if (remain.startsWith("&#45") && !remain.startsWith("&#45O&#45")) {
			print_image(img.dash, offset);
			remain = remain.replace("&#45", "");
			offset += 7;
		} else if (remain.startsWith("&#45O&#45")) {
			print_image(img.dash_hole, offset);
			remain = remain.replace("&#45O&#45", "");
			offset += 7;
		} else if (remain.startsWith("_|&#8254")) {
			...
			remain = remain.replace("||", "");
			offset += 5;
		} else if (remain.startsWith("&#135&#135")) {
